...
use crate::scan::SOCKET_CON_TIMEOUT_MS;
...
// pub const MOST_COMMON_PORTS_100: &[u16] = &[80, 23, 443, 21]; // FOR DEV
...
#[instrument(name = "on", level = "info", fields(domain = subdomain.domain), skip_all)]
pub async fn scan_ports(concurrency: usize, mut subdomain: Subdomain) -> Subdomain {
    let socket_addresses: Vec<SocketAddr> =
        match format!("{}:1024", subdomain.domain).to_socket_addrs() {
            ...
            Err(_) => {
                error!("\"{}:1024\" socket cannot be created", subdomain.domain);
                Vec::new()
            }
        };
...
#[instrument(name = "port", level = "debug", fields(ip = socker_address.ip().to_string() ,port = port), skip_all)]
